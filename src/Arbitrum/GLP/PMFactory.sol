...
contract PMfactory {
    ...
    function deployNewContest(
        ...
        newContest = address(new PMcontest(_depositDeadline,_GLPaddress,_withdrawStart,_rewardRouter,_wethAddress));
        id = contestCount++;
        isContestRunning[id] = true;
        contestAddress[id] = newContest;
    }

    function contestSetWinner(uint256 _id) onlyOwner external returns(uint winner){
